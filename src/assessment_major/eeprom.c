 ...
 */
static void calculateSimpleChecksum(EEPROMIntersectionDataStructBuffer *buffer) {
    uint16_t sum = 0;
    ...
    const uint8_t buffSize = sizeof(EEPROMIntersectionDataStructBuffer) - sizeof(uint16_t);
    for (int i = 0; i < buffSize; i++) {
        sum += buffer->buffer[i];
    }
    buffer->intersectionDataStruct.internal_checksum = sum;
}
...
void EEPROM_SetIntersectionDataStruct(IntersectionDataStruct intersectionData) {
    ...
    eepromState.idf          = (EEPROMIntersectionDataStructBuffer){intersectionData};
    // Reset written bytes back to 0
    eepromState.bytesWritten = 0;
    // Calculate the checksum for data
    calculateSimpleChecksum(&(eepromState.idf));
...
EEPROMReadIntersectionData EEPROM_ReadIntersectionData() {
    EEPROMIntersectionDataStructBuffer dataBuffer;

    for (uint16_t i = 0; i < sizeof(dataBuffer.buffer); i++) {
        EEPROM_ReadByte(i, &dataBuffer.buffer[i]);
    }

    uint16_t eepromReadChecksum = dataBuffer.intersectionDataStruct.internal_checksum;
...
void EEPROM_WriteNextByte() {
    ...
    if (eepromState.bytesWritten < sizeof(EEPROMIntersectionDataStructBuffer)) {
        ...
        if (EEPROM_UpdateByte(eepromState.bytesWritten, eepromState.idf.buffer[eepromState.bytesWritten])) {
